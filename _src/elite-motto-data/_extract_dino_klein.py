#!/usr/bin/env python3
"""
Extract Dino 3-5 Elite page (Phase C #6: safari-mittel, NO ritual block)
...
SRC = 'kindergeburtstag/dino-3-5-jahre.html'
...
# ============================================================
age_match = re.search(
    r'<h3[^>]*>🧠 Was 3–5-Jährige ausmacht.*?</h3>(.*?)<h3',
    html, re.DOTALL)
if age_match:
...
def extract_variant(variant_id, variant_label):
    ...
    for i, (h4_pos, h4_m) in enumerate(h4_starts):
        ...
        for key, val in markers.items():
            k = key.lower()
            if 'für 3' in k or '3-jährig' in k: age6 = val
            elif 'für 4' in k or '4-jährig' in k: age7 = val
            elif 'für 5' in k or '5-jährig' in k: age8 = val
...
# Write result
# ============================================================
out_path = '_src/elite-motto-data/dino-klein.json'
