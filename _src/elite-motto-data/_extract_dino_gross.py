#!/usr/bin/env python3
"""
Extract Dino 9-12 Elite page (Phase C #6: safari-mittel, NO ritual block)
Strategy: parse HTML systematically, mark anything not in source as null with note.
"""
import re
import json
import sys
...
SRC = 'kindergeburtstag/dino-9-12-jahre.html'
...
# Unescape numeric HTML entities (emoji) BUT NOT structural (lt/gt/amp)
import re as _re
html = _re.sub(r'&#x([\dA-Fa-f]+);', lambda m: chr(int(m.group(1), 16)), html)
html = _re.sub(r'&#(\d+);', lambda m: chr(int(m.group(1))), html)
...
result = {
    "_meta": {
        ...
        "purpose": "GOLDEN TEMPLATE — Phase A extraction for Planer-Frisur-Sprint (P3-13 to P3-19)",
...
# ============================================================
age_match = re.search(
    r'<h3[^>]*>🧠 Was 9–12-Jährige ausmacht.*?</h3>(.*?)<h3',
    html, re.DOTALL)
if age_match:
...
def extract_variant(variant_id, variant_label):
    ...
    for i, (h4_pos, h4_m) in enumerate(h4_starts):
        ...
        for key, val in markers.items():
            k = key.lower()
            if 'für 9' in k or '9-jährig' in k: age6 = val
            elif 'für 10' in k or '10-jährig' in k: age7 = val
            elif 'für 11' in k or '11-jährig' in k or 'für 12' in k: age8 = val
...
result['preparationWeeks'] = {
    ...
    '_promptForPhaseB': 'Schreibe 6 Zeitfenster (minus4Weeks, minus2Weeks, minus1Week, minus2Days, minus1Day, dayOf) mit konkreten Action-Items für Feuerwehr-6-8-Party.'
}
result['sosScenarios'] = {
    '_status': 'TODO_PHASE_B',
...
# Write result
# ============================================================
out_path = '_src/elite-motto-data/dino-gross.json'
