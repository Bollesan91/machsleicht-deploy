# Sicherheitshinweise: dino

Drei Altersgruppen (3-5, 6-8, 9-12), je drei Aufwandsvarianten.
**73 Einkaufsposten**, davon **53 mit gedruckter Regel** und **20 mit einer Begruendung, warum keine noetig ist** — zusammen alle 73, kein unentschiedener Posten.
Auf den drei oeffentlichen Seiten stehen **62 Regeln** gedruckt (weniger als 53, weil die freien Seiten nicht jeden Katalogposten verkaufen).

---

## 3-5 Jahre — Katalog


### Variante minimal — 4 Regeln, 1 als harmlos entschieden

- **Mini-Dinos Set (12–24 Stück)**
  > Sortier die Figuren vor der Party durch: Was durch eine Klopapierrolle passt, ist bei 3-Jährigen Erstickungsgefahr — kleine Exemplare bleiben in der Schachtel. Nach Ausgrabung und Eier-Suche zählst du sie zurück, damit keine unterm Sofa liegen bleibt.

- **Plastik-Eier bunt (20 Stk.)**
  > Die Plastik-Eier öffnest du oder ein Kind unter deinen Augen — dünnwandige Eier brechen beim Aufhebeln in scharfe Splitter, die im Sand nicht mehr auffallen. Gesprungene Hälften kommen sofort weg, nicht zurück in die Kiste.

- **Getränke + Snacks**
  > Trauben und Beeren werden längs halbiert oder geviertelt, bevor sie auf den Tisch kommen — im Ganzen verschließen sie die Atemwege eines Kleinkinds komplett. Gegessen wird im Sitzen.

- **Mitgebsel (Dinos + Süßes)**
  > Die Tüten bleiben bei dir und gehen erst beim Abschied raus: Mini-Dino und zähe Gummibärchen sind bei 3- bis 5-Jährigen Verschluck- und Erstickungsgefahr, sobald sie zwischendurch im Spiel geöffnet werden.


**Ohne Regel, mit Begruendung:**

- *Kuchen-Zutaten* — Backzutaten, die Erwachsene zum Vulkan-Kuchen verarbeiten; verdeckte Allergene sind durch die im Plan verankerte Allergie-Abfrage (Einladung/parentTips) abgedeckt.

### Variante standard — 7 Regeln, 0 als harmlos entschieden

- **Dino-Figuren Set (12+ Stück)**
  > Sortier die Figuren vor der Party durch: Was durch eine Klopapierrolle passt, ist bei 3-Jährigen Erstickungsgefahr — kleine Exemplare bleiben in der Schachtel. Nach Ausgrabung und Eier-Suche zählst du sie zurück, damit keine unterm Sofa liegen bleibt.

- **Plastik-Eier (20 Stk.)**
  > Die Plastik-Eier öffnest du oder ein Kind unter deinen Augen — dünnwandige Eier brechen beim Aufhebeln in scharfe Splitter, die im Sand nicht mehr auffallen. Gesprungene Hälften kommen sofort weg, nicht zurück in die Kiste.

- **Sand/Reis für Ausgrabung**
  > Reis und Sand bleiben in der Wanne und die Kinder sitzen beim Graben — in dem Alter wandern Körner in Mund, Nase und Ohr, und ein Reiskorn in der Nase endet beim Arzt. Ein Erwachsener sitzt die ganze Station über daneben.

- **Dino-Stempel Set**
  > Mini-Stempel sind Kleinteile: Sie bleiben auf dem Tisch und werden nach der Station eingesammelt. Nimm Stempelkissen auf Wasserbasis, die als spielzeugtauglich gekennzeichnet sind — gestempelt wird auf Papier, nicht auf Haut.

- **Vulkan-Material (Essig, Natron, Spüli, Farbe)**
  > Den Essig kippt eine erwachsene Person ein, nicht das Geburtstagskind — die Kinder stehen eine Armlänge zurück und schauen nie von oben in den Krater. Essigflasche und Natron nach der Demo sofort außer Reichweite räumen; Spritzer im Auge: sofort mindestens 10 Minuten mit lauwarmem Wasser ausspülen, Lid sanft offen halten — lieber zu lang als zu kurz, danach den Augenarzt anrufen.

- **Kuchen + Getränke + Snacks**
  > Trauben und Beeren werden längs halbiert oder geviertelt, Popcorn gibt es erst ab etwa vier Jahren — beides sind bei 3- bis 5-Jährigen die häufigsten Erstickungsursachen. Gegessen wird am Tisch im Sitzen, nicht auf der Ausgrabung.

- **Mitgebsel**
  > Die Tüten bleiben bei dir und gehen erst beim Abschied raus: Mini-Dino und zähe Gummibärchen sind bei 3- bis 5-Jährigen Verschluck- und Erstickungsgefahr, sobald sie zwischendurch im Spiel geöffnet werden.


### Variante wow — 9 Regeln, 0 als harmlos entschieden

- **Dino-Figuren Set (12+ Stück)**
  > Sortier die Figuren vor der Party durch: Was durch eine Klopapierrolle passt, ist bei 3-Jährigen Erstickungsgefahr — kleine Exemplare bleiben in der Schachtel. Nach Ausgrabung und Eier-Suche zählst du sie zurück, damit keine unterm Sofa liegen bleibt.

- **Plastik-Eier (20–25 Stk.)**
  > Die Plastik-Eier öffnest du oder ein Kind unter deinen Augen — dünnwandige Eier brechen beim Aufhebeln in scharfe Splitter, die im Sand nicht mehr auffallen. Gesprungene Hälften kommen sofort weg, nicht zurück in die Kiste.

- **Sand/Reis für Ausgrabung**
  > Reis und Sand bleiben in der Wanne und die Kinder sitzen beim Graben — in dem Alter wandern Körner in Mund, Nase und Ohr, und ein Reiskorn in der Nase endet beim Arzt. Ein Erwachsener sitzt die ganze Station über daneben.

- **Dino-Stempel Set**
  > Mini-Stempel sind Kleinteile: Sie bleiben auf dem Tisch und werden nach der Station eingesammelt. Nimm Stempelkissen auf Wasserbasis, die als spielzeugtauglich gekennzeichnet sind — gestempelt wird auf Papier, nicht auf Haut.

- **Dino-Tattoos**
  > Klebetattoos testest du erst an einer kleinen Stelle am Unterarm und wartest ein paar Minuten — Kleinkindhaut reagiert häufiger. Nicht ins Gesicht und nicht auf gerötete Haut; beim ersten Jucken mit Öl ablösen.

- **Vulkan-Material**
  > Den Essig kippt eine erwachsene Person ein, nicht das Geburtstagskind — die Kinder stehen eine Armlänge zurück und schauen nie von oben in den Krater. Essigflasche und Natron nach der Demo sofort außer Reichweite räumen; Spritzer im Auge: sofort mindestens 10 Minuten mit lauwarmem Wasser ausspülen, Lid sanft offen halten — lieber zu lang als zu kurz, danach den Augenarzt anrufen.

- **Kuchen + Getränke + Snacks**
  > Trauben und Beeren werden längs halbiert oder geviertelt, Popcorn gibt es erst ab etwa vier Jahren — beides sind bei 3- bis 5-Jährigen die häufigsten Erstickungsursachen. Gegessen wird am Tisch im Sitzen, nicht auf der Ausgrabung.

- **Deko (Ballons, Papier)**
  > Aufgepustet wird von dir — beim Selbst-Aufblasen kann ein Kind den Ballon einatmen. Geplatzte Fetzen und nicht aufgeblasene Ballons sammelst du sofort restlos ein: Von allen Gegenständen ersticken Kleinkinder am häufigsten an Luftballons.

- **Mitgebsel**
  > Die Tüten bleiben bei dir und gehen erst beim Abschied raus: Mini-Dino und zähe Gummibärchen sind bei 3- bis 5-Jährigen Verschluck- und Erstickungsgefahr, sobald sie zwischendurch im Spiel geöffnet werden.


---

## 6-8 Jahre — Katalog


### Variante minimal — 3 Regeln, 4 als harmlos entschieden

- **Dino-Figuren (12 Stk.)**
  > Für die Geburtstagsrunde sind die Figuren harmlos, für mitfeiernde Geschwister unter 3 nicht: Sie liegen bei Ausgrabung und Eier-Suche im ganzen Raum verteilt. Nach dem Spiel einsammeln und zählen, bevor die Kleinen wieder rein dürfen.

- **Popcorn-Mais + Snacks**
  > Popcorn bleibt am Tisch: Für mitfeiernde Geschwister unter vier ist es ein klassisches Aspirationsrisiko, ungepoppte Körner noch mehr. Offene Schalen kommen nicht auf den Boden oder in die Spielecke.

- **Backmischung + bunte Schoko-Linsen**
  > Frag Milch-, Ei- und Nussallergien mit der Einladung ab — Backmischung, Schokoglasur und Schoko-Linsen bringen Milch und oft Nussspuren mit. Stellst du eine Dino-Figur aus Plastik oben drauf, nimm sie vor dem Anschneiden herunter.


**Ohne Regel, mit Begruendung:**

- *Saftpäckchen (10er Pack)* — Getränk am Partytisch für 6-8-Jährige — kein Risiko, das eine Elternregel braucht.
- *Ausmalbilder* — Papier-Ausdrucke als Tisch-Aktivität für 6-8-Jährige — kein Risiko.
- *Gummibärchen (Mitgebsel)* — Altersgerechte Süßigkeit für 6-8-Jährige als Mitgebsel bzw. Trostpreis am Tisch — kein reales Risiko in dieser Altersgruppe.
- *Papiertüten (DM/Action)* — Papiertüten als Mitgebsel-Verpackung, vom Erwachsenen gepackt — kein Risiko.

### Variante standard — 7 Regeln, 2 als harmlos entschieden

- **Dino-Figuren (12 Stk.)**
  > Für die Geburtstagsrunde sind die Figuren harmlos, für mitfeiernde Geschwister unter 3 nicht: Sie liegen bei Ausgrabung und Eier-Suche im ganzen Raum verteilt. Nach dem Spiel einsammeln und zählen, bevor die Kleinen wieder rein dürfen.

- **Gipspulver 1 kg (Baumarkt)**
  > Den Gips rührst du an, nicht die Kinder, und der Staub darf nicht eingeatmet werden. Nur in offene Becher gießen — nie um Hände oder Finger, der Gips wird beim Abbinden heiß; Reste antrocknen lassen und in den Restmüll, nie ins Waschbecken.

- **Dino-Teller & Becher Set**
  > Für Vulkan und Gips nimmst du Becher aus der Küche, nie das Dino-Geschirr vom Tisch — Essig-Schaum und Gipswasser gehören nicht in einen Trinkbecher. Deck den Tisch erst, wenn die Experimente abgeräumt sind.

- **Grüne Luftballons (10 Stk.)**
  > Ballons pustest du auf — beim Selbst-Aufblasen kann ein Kind den Ballon einatmen. Geplatzte Ballons und Gummifetzen sofort komplett einsammeln, bevor jüngere Geschwister sie in den Mund nehmen.

- **Saftpäckchen + Snacks + Würstchen**
  > Mini-Würstchen schneidest du längs und dann in Stücke, nie in runde Scheiben — rund und fest ist die Form, an der Kleinkinder ersticken. Für mitfeiernde Geschwister gilt das ohne Ausnahme.

- **Backmischung + Deko**
  > Frag mit der Einladung nach Milch-, Ei- und Nussallergien — Backmischung, Schokoglasur und Schoko-Linsen bringen Milch und oft Nussspuren mit. Die Plastik-Dinos auf dem Kuchen nimmst du vor dem Anschneiden herunter, sie sind kein Essen.

- **Lebensmittelfarbe (Supermarkt)**
  > Färb Kuchen und Vulkan mit getrennten Löffeln und stell die Vulkan-Schale weit weg vom Essenstisch — rot geschäumtes Essig-Spülmittel-Gemisch sieht aus wie Nachtisch. Den Essig kippst du rein, die Kinder bleiben eine Armlänge zurück.


**Ohne Regel, mit Begruendung:**

- *Dino-Masken zum Ausmalen* — Papp-Masken zum Bemalen am Tisch als Ankunfts-Bastelei für 6-8-Jährige — Karnevals-Standard ohne reales Risiko.
- *Papiertüten + Gummibänder* — Verpackungsmaterial, das Erwachsene zum Packen der Mitgebsel-Tüten nutzen — kein Risiko.

### Variante wow — 10 Regeln, 5 als harmlos entschieden

- **Dino-Figuren (12 Stk.)**
  > Für die Geburtstagsrunde sind die Figuren harmlos, für mitfeiernde Geschwister unter 3 nicht: Sie liegen bei Ausgrabung und Eier-Suche im ganzen Raum verteilt. Nach dem Spiel einsammeln und zählen, bevor die Kleinen wieder rein dürfen.

- **Gipspulver 1 kg (Baumarkt)**
  > Den Gips rührst du an, nicht die Kinder, und der Staub darf nicht eingeatmet werden. Nur in offene Becher gießen — nie um Hände oder Finger, der Gips wird beim Abbinden heiß; Reste antrocknen lassen und in den Restmüll, nie ins Waschbecken.

- **Dino-Teller & Becher Set**
  > Das Dino-Geschirr bleibt Essgeschirr: Vulkan-Essig und Gips rührst du in Joghurtbechern an, die danach in den Müll gehen. Stell die Experimente auf ein eigenes Tablett und räum sie ab, bevor Pizza und Kuchen auf den Tisch kommen.

- **Luftballons + Krepppapier-Lianen**
  > Ballons pustest du auf — beim Selbst-Aufblasen kann ein Kind den Ballon einatmen. Geplatzte Ballons und Gummifetzen sofort komplett einsammeln, bevor jüngere Geschwister sie in den Mund nehmen.

- **Lupen-Set (6 Stk.)**
  > Lupen bündeln Sonnenlicht zu einem Brennpunkt: Draußen bleiben sie in der Tasche, solange die Sonne scheint, und gegraben wird ohne sie. Gehen sie als Mitgebsel mit, sag den Eltern beim Abschied kurz Bescheid.

- **Saftpäckchen + Snacks + Würstchen**
  > Mini-Würstchen schneidest du längs und dann in Stücke, nie in runde Scheiben — rund und fest ist die Form, an der Kleinkinder ersticken. Für mitfeiernde Geschwister gilt das ohne Ausnahme.

- **Backmischung + Deko + Wunderkerze**
  > Nimm eine lebensmittelechte Kuchen-Wunderkerze — normale lassen Rückstände auf den Kuchen rieseln. Die Wunderkerze zündest nur du an, die Kinder bleiben sitzen: mindestens 30 cm Abstand zu Haaren, Kleidung, Vorhängen und Papierdeko. Ein Glas nimm ein Stabfeuerzeug statt Streichholz, Wasser danebenstellen und die Kerze direkt nach dem Abbrennen darin ablöschen. Nach dem Abbrennen kommt die Kerze vom Kuchen, bevor angeschnitten wird.

- **Lebensmittelfarbe (Supermarkt)**
  > Stell den Vulkan auf ein eigenes Tablett, weit weg vom Kuchentisch: Der rot gefärbte Schaum aus Essig und Spülmittel sieht aus wie Essen. Den Essig kippst du selbst rein, die Kinder stehen eine Armlänge zurück.

- **Schoko-Goldmünzen**
  > Wenn jüngere Geschwister mitlaufen: Münzen nicht in Krabbelhöhe verstecken und nach der Schatzsuche nachzählen, damit keine liegen bleibt. Goldfolie vor dem Essen ganz abziehen und einsammeln.

- **T-Rex-Kostüm (optional)**
  > Das aufblasbare Kostüm nimmt dem Träger fast das ganze Sichtfeld: Damit wird nicht gerannt und nicht gefangen — der T-Rex stapft, die anderen laufen. Auf freier Fläche ohne Tischkanten, und für jüngere Kinder vorher ankündigen, damit niemand erschrickt.


**Ohne Regel, mit Begruendung:**

- *Dino-Masken zum Ausmalen* — Papp-Masken zum Bemalen am Tisch für 6-8-Jährige — Karnevals-Standard ohne reales Risiko.
- *Dino-Stempel (für Forscherpass)* — Stempeln des Forscherpasses am Pult unter Erwachsenen-Regie für 6-8-Jährige — Bastel-Standard ohne reales Risiko.
- *Kartoffeln (Vulkan-Brei)* — Normales Lebensmittel, von Erwachsenen gekocht und als Kartoffelbrei-Vulkan serviert — kein Risiko.
- *Forscherpass + Quiz* — Papier-Druckvorlagen für Pass und Quiz am Tisch — kein Risiko.
- *Papiertüten + Gummibänder* — Verpackungsmaterial, das Erwachsene zum Packen der Mitgebsel-Tüten nutzen — kein Risiko.

---

## 9-12 Jahre — Katalog


### Variante minimal — 3 Regeln, 2 als harmlos entschieden

- **Dino-Figuren Set**
  > Für die Geburtstagsrunde sind die Figuren harmlos, für mitfeiernde Geschwister unter 3 nicht: Sie liegen bei Ausgrabung und Eier-Suche im ganzen Raum verteilt. Nach dem Spiel einsammeln und zählen, bevor die Kleinen wieder rein dürfen.

- **Vulkan-Material (Essig, Natron, Farbe)**
  > Den Essig kippt eine erwachsene Person ein, alle anderen bleiben eine Armlänge zurück und schauen nicht von oben in den Krater. Nur die Natron-Essig-Variante — kein Wasserstoffperoxid, kein Trockeneis; Essig im Auge: sofort mindestens 10 Minuten mit lauwarmem Wasser spülen, Lid sanft offen halten, vom inneren Augenwinkel nach außen — lieber zu lang als zu kurz, danach den Augenarzt anrufen.

- **Mitgebsel**
  > Halt Mini-Dino und Bonbons an eine Klopapierrolle, bevor die Tüte zugeht: Was durchpasst, ist für Kinder unter drei gefährlich. Zuhause wird die Tüte ohne Aufsicht geöffnet — sag den Eltern beim Abschied kurz Bescheid.


**Ohne Regel, mit Begruendung:**

- *Essen + Getränke + Kuchen* — Pizza, Popcorn und Kuchen als Tischessen für 9-12-Jährige — altersgerechte Esssituation, Allergie-Abfrage (Pizza-Beläge) ist im Plan verankert.
- *Quiz (Druckvorlage)* — Papier-Druckvorlage für die Quiz-Runde am Tisch — kein Risiko.

### Variante standard — 4 Regeln, 3 als harmlos entschieden

- **Dino-Figuren Set (12+ Stk.)**
  > Für die Geburtstagsrunde sind die Figuren harmlos, für mitfeiernde Geschwister unter 3 nicht: Sie liegen bei Ausgrabung und Eier-Suche im ganzen Raum verteilt. Nach dem Spiel einsammeln und zählen, bevor die Kleinen wieder rein dürfen.

- **Ausgrabungs-Zubehör (Pinsel, Sand)**
  > Gehämmert und gemeißelt wird nur im Sitzen am Tisch, mit Abstand zu den anderen, und wer zuschaut, bleibt zwei Meter weg — Gipssplitter fliegen weiter als man denkt. Wenn dein Set Hammer und Meißel enthält, schlägst du den Block vor, die Kinder pinseln frei.

- **Vulkan-Material**
  > Den Essig kippt eine erwachsene Person ein, alle anderen bleiben eine Armlänge zurück und schauen nicht von oben in den Krater. Nur die Natron-Essig-Variante — kein Wasserstoffperoxid, kein Trockeneis; Essig im Auge: sofort mindestens 10 Minuten mit lauwarmem Wasser spülen, Lid sanft offen halten, vom inneren Augenwinkel nach außen — lieber zu lang als zu kurz, danach den Augenarzt anrufen.

- **Mitgebsel**
  > Prüf Dino-Figur, Fossil-Stein und Bonbons mit der Klopapierrolle, bevor die Tüte zugeht — was durchpasst, ist für Kinder unter drei gefährlich. Sag den Eltern beim Abschied Bescheid, damit sie die Tüte zuhause zuerst öffnen.


**Ohne Regel, mit Begruendung:**

- *Dino-Stempel (für Forscherpass)* — Stempeln des Forscherpasses am Pult durch 9-12-Jährige — Bastel-Standard ohne reales Risiko.
- *Essen + Getränke + Kuchen* — Pizza, Hot Dogs, Popcorn und Kuchen als Tischessen für 9-12-Jährige — altersgerecht, Allergie-Abfrage ist im Plan verankert.
- *Forscherpass + Quiz* — Papier-Druckvorlagen für Pass und Quiz am Tisch — kein Risiko.

### Variante wow — 6 Regeln, 3 als harmlos entschieden

- **Dino-Figuren Set**
  > Für die Geburtstagsrunde sind die Figuren harmlos, für mitfeiernde Geschwister unter 3 nicht: Sie liegen bei Ausgrabung und Eier-Suche im ganzen Raum verteilt. Nach dem Spiel einsammeln und zählen, bevor die Kleinen wieder rein dürfen.

- **Ausgrabungs-Zubehör**
  > Gehämmert und gemeißelt wird nur im Sitzen am Tisch, mit Abstand zu den anderen, und wer zuschaut, bleibt zwei Meter weg — Gipssplitter fliegen weiter als man denkt. Wenn dein Set Hammer und Meißel enthält, schlägst du den Block vor, die Kinder pinseln frei.

- **Zahlenschloss 5-stellig**
  > Nimm ein leichtes Zahlen-Fahrradschloss statt eines schweren Vorhängeschlosses — das springt beim Öffnen auf und fällt gern auf Finger oder Füße. Häng es locker durch den Deckel und stell die Truhe zum Knacken flach auf den Boden.

- **Mini-Lupen (8 Stk.)**
  > Lupen bündeln Sonnenlicht zu einem Brennpunkt: Draußen bleiben sie in der Tasche, solange die Sonne scheint, und gegraben wird ohne sie. Gehen sie als Mitgebsel mit, sag den Eltern beim Abschied kurz Bescheid.

- **Vulkan-Material**
  > Den Essig kippt eine erwachsene Person ein, alle anderen bleiben eine Armlänge zurück und schauen nicht von oben in den Krater. Nur die Natron-Essig-Variante — kein Wasserstoffperoxid, kein Trockeneis; Essig im Auge: sofort mindestens 10 Minuten mit lauwarmem Wasser spülen, Lid sanft offen halten, vom inneren Augenwinkel nach außen — lieber zu lang als zu kurz, danach den Augenarzt anrufen.

- **Mitgebsel**
  > Halt jedes Teil an eine Klopapierrolle, bevor es in die Tüte kommt — Fossil-Stein, Dino-Figur und Bonbons passen durch und sind für Kinder unter drei gefährlich. Bitte die Eltern beim Abschied, die Tüte zuhause selbst zu öffnen.


**Ohne Regel, mit Begruendung:**

- *Dino-Stempel* — Stempeln des Forscherpasses am Pult durch 9-12-Jährige — Bastel-Standard ohne reales Risiko.
- *Essen + Getränke + Kuchen* — Pizza, Nachos, Popcorn und Kuchen als Tischessen für 9-12-Jährige — altersgerecht, Allergie-Abfrage ist im Plan verankert.
- *Forscherpass + Quiz + Rätsel* — Papier-Druckvorlagen für Pass, Quiz und Escape-Rätsel am Tisch — kein Risiko.
