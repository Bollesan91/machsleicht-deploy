...
function rexState(t,i){ if(t<9.6) return null; let x,y,dir=1,sc=1,mouth=0,happy=false,blush=0,sniff=0,run=false,behind=false,dash=0,lift=0;
  if(t<10.6){ const u=easeOut(seg(t,9.6,10.6)); x=lerp(1500,REX_X,u); y=GROUND_Y; run=true; lift=160; }
  else if(t<13.3){ x=REX_X; y=GROUND_Y; lift=160; if(t>=10.8&&t<11.6) mouth=Math.sin(seg(t,10.8,11.6)*Math.PI); if(t>=11.9&&t<12.4) sniff=Math.sin(seg(t,11.9,12.4)*Math.PI); if(t>=12.4){ happy=true; blush=seg(t,12.4,12.9); if(t>=12.9) y=GROUND_Y-12*Math.abs(Math.sin((t-12.9)*12)); } }
  else if(t<16.6){ const mx=mountX(t); y=GROUND_Y; happy=true; blush=0.6; behind=true; run=true;
    if(t<14.1){ x=lerp(REX_X,-470,seg(t,13.3,14.1)); dir=1; dash=(t-13.3)*3; lift=160+140*seg(t,13.3,13.6); }              /* saust hinter Trixi vorbei nach links aus dem Bild */
  ...
  return {x,y,dir,sc,mouth,happy,blush,sniff,run,behind,dash,lift}; }
function rex(d,t,i){ ctx.save(); ctx.translate(d.x,d.y); ctx.scale(d.dir*d.sc,d.sc); const ph=STEP[i]*1.3+d.dash; const wag=d.happy?0.25*Math.sin(t*8):0;
  ...
  /* Hals + Kopf */ ctx.save(); ctx.translate(-130,-330); ctx.rotate(d.sniff*0.35); ctx.strokeStyle='#6B8E4E'; ctx.lineWidth=90; ctx.beginPath(); ctx.moveTo(0,0); ctx.lineTo(-110,-90-d.lift); ctx.stroke(); ctx.translate(0,-d.lift);
...
function pia(t,i){ const fl=Math.sin(t*10)*0.6; if(t<6.2) return null;
  ...
  if(t<11.3){ const p=mountPoint(0,-640,10.8,i); const u=easeOut(seg(t,10.8,11.3)); return {x:lerp(p.x,560,u),y:lerp(p.y-20,440,u),dir:1,flap:fl,perched:false}; }
  if(t<14.0) return {x:560+16*Math.sin(t*2.2),y:440+14*Math.sin(t*3.1),dir:1,flap:fl,perched:false};
  if(t<16.5){ const mx=mountX(t); const u=easeInOut(seg(t,14.0,14.6)); return {x:lerp(560,mx-260,u)+10*Math.sin(t*3),y:lerp(440,600,u)+12*Math.sin(t*2.6),dir:1,flap:fl,perched:false}; }
...
function texts(t,i){ const name=nameVal();
  ...
  if(t>=12.4&&t<13.4) bubble(700,500,360,110,'Party?!',Math.min(seg(t,12.4,12.7),1-seg(t,13.1,13.4)),'#A63F25','#E8C15A');
