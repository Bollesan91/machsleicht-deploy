...
function mount(t,i){ const mx=mountX(t), my=mountY(t,i); const flagUp=Math.min(seg(t,11.8,12.2),1-seg(t,13.4,13.8)); const arm=armState(t); const moving=speed(t)>150;
  ...
  /* Geschlossene Kabine: Wand, dunkler Sockel, Dach, Tuer */ ctx.fillStyle='#F6C21A'; rrect(-190,-625,196,395,16); ctx.fill(); ctx.save(); rrect(-190,-625,196,395,16); ctx.clip(); ctx.fillStyle='#D9A400'; ctx.fillRect(-190,-300,196,70); ctx.restore(); ctx.fillStyle='#F6C21A'; rrect(-200,-636,216,26,8); ctx.fill(); ctx.strokeStyle='#D9A400'; ctx.lineWidth=4; rrect(-170,-345,150,100,10); ctx.stroke(); ctx.fillStyle='#4A5059'; rrect(-60,-302,26,8,3); ctx.fill();
  /* Fenster: Innenraum, Kind schaut heraus (auf das Fenster beschnitten) */ const WX=-170, WY=-598, WW=156, WH=230; ctx.fillStyle='#2A3A4A'; rrect(WX,WY,WW,WH,14); ctx.fill();
  ctx.save(); rrect(WX,WY,WW,WH,14); ctx.clip(); ctx.fillStyle='#2F6DB5'; rrect(-150,-412,124,80,24); ctx.fill(); ctx.fillStyle='#F28C28'; rrect(-140,-410,104,120,26); ctx.fill(); ctx.fillStyle='#D9DDE3'; ctx.fillRect(-140,-386,104,10);
  ctx.fillStyle='#F6C9A4'; rrect(-102,-426,28,26,8); ctx.fill(); ctx.save(); ctx.beginPath(); ctx.arc(-88,-470,56,0,6.29); ctx.clip(); ctx.fillStyle='#F6C9A4'; ctx.fillRect(-148,-530,120,120); drawPhotoInCircle(-88,-470,56,zoomVal()); ctx.restore(); ctx.strokeStyle='#D9A400'; ctx.lineWidth=4; ctx.beginPath(); ctx.arc(-88,-470,56,0,6.29); ctx.stroke(); helmet(-88,-516,0,2.05); ctx.restore();
  /* Hand am Fensterrand, Rahmen, Glanz */ dots([[-50,-372,13]],'#F6C9A4'); ctx.strokeStyle='#F6C9A4'; ctx.lineWidth=6; ctx.lineCap='round'; for(const dx of [-8,0,8]){ ctx.beginPath(); ctx.moveTo(-50+dx,-378); ctx.lineTo(-50+dx,-392); ctx.stroke(); } ctx.strokeStyle='#4A5059'; ctx.lineWidth=8; rrect(WX,WY,WW,WH,14); ctx.stroke(); ctx.save(); rrect(WX,WY,WW,WH,14); ctx.clip(); ctx.fillStyle='rgba(255,255,255,0.16)'; ctx.beginPath(); ctx.moveTo(WX+104,WY); ctx.lineTo(WX+WW,WY); ctx.lineTo(WX+WW,WY+66); ctx.closePath(); ctx.fill(); ctx.restore();
