...
- #2 lianen-dschungel: Schmiede-Pass 1eb5d95 (Flut-Reveal, Doppel-Lianen ab 7 mit 450ms-Nachgriff, **Story-Pilot: Koko-Intro-Panels + Story-Outro**), U6-Zonenrand-Fix 8016cd1. Gate-R1 2e693e73: Story-Konzept GO, Twist-Konzept GO/Implementierung NO. Gate-R2 fabc09b9 **NO-GO** (Outro-Lesbarkeit, GUEST-Quelle) → Tap-Gating/guestNow/kidCap in 75c372d. Gate-R3 (frischer Chat 77106c09, auf 75c372d) **NO-GO 78/100**, aber „danach 90+-Kandidat": MAJOR-1 Reflex-Tap überspringt Flood+Beats (Doppel-Lianen trainieren Schnell-Taps, advanceOutro ungeschützt) → **900ms-Grace-Fenster** (winAt); MAJOR-2 NOPHOTO-Story-Bruch (Intro macht GAST zum Kletterer, Outro lobt kid()) → nofoto-Beats auf DU umgestellt + Boss-Rätsel aufgelöst („ein freches Äffchen"); MINOR: Fallback 8s→11s, schließendes „…“-Anführungszeichen, .field/.land in core.css touch-action. Positiv-Zitate R3: „liegt im Erlebnis über beiden Ankern", Twist „stärker als beide Referenzen". **Gate-R4 (frischer Chat 30997eae, auf b4fa8a7): GATE-GO 88/100 — 0 MAJORs, Pflichttests a–f alle JA (beide Pfade), „sauberster Finale-Bau der drei Spiele".** Danach Politur-Pass der 8 R4-MINORs (deterministisch, Diff-Re-Check statt Vollrunde): Hut-Slide-in via sharpen() (Hütte war nie sichtbar — right:-14% + overflow:hidden), kidCap im nofoto-winWho, Beat-2-Fallback 14s, GUEST >20 verwerfen statt kürzen, .vine.caught::before opacity 0, spawnInitT-clearTimeout gegen Start-Doppeltipp, tip() sammelt Prä-Miss-Band ein, ▸-Affordanz blinkt. 2 UNSICHER (Kasus-Apposition, Blind-Tipp-Decke) → OFFENE-REVIEW-PUNKTE. Dabei Lektion (7) live bestätigt: //-Kommentar mitten in tip()-Einzeiler → SyntaxError, von node --check gefangen. Sweeps 5+11 grün, Hütten-Einfahrt per Screenshot verifiziert. ⚠️ Fable-Limit 99 %, Reset Do 10.07. 14:00 — huerden-Gate auf **Opus 4.8 Max** (Memory-Fallback).
- #3 huerden-pferde: **GATE-GO** — R1 (Chat 6f3d2f11, Opus 4.8 Max auf b4fa8a7) **88/100, 0 MAJOR**, „sauberster Finale-Bau der drei", lobte fälschlich das Umkippen als Jubel-Stelle. Danach 8 R4-Lektionen gespiegelt (846437e→75772bf) + Bolle-Feedback 03.07.: (A) **🐎 scaleX(-1)** in Basis+leap+stumble (galoppiert vorwärts), (B) **.knock→.cleared + „+1 🏅"-pop1** (Hürde bleibt STEHEN — Umkippen ist Springreit-Fehler), a7e3945; dann 4 R1-MINOR-Politur (Grace 900→1150, „Siegerbild"→„Champion-Foto", winAt-Reset, toter HORSE_X) 6f6020a. **Re-Gate R2 (Chat ae2d2611, Opus Max auf 6f6020a): 87/100, 0 MAJOR, GO** — (A)+(B) „tragen", Flip = korrekter Bascule (Nase runter überm Sprung), pop1 kein DOM-Leak; Foto-„Champion ist kid" als MINOR „narrativ gedeckt, kein Blocker" (nofoto bleibt „…bist DU"). Sweeps 5+11 grün, Screenshots verifiziert. **DURCH.**
- #4 spuren-safari (Story-Nachrüstung nach GATE-GO 88): Leo-Panels mit falscher Fährte + Tap-Outro (8a3db37). Stufe-2-Gate (Chat 0d7be59f) **GO 93** — ABER Prompt hatte „bestand GO 88" drin → Bolle-Catch „du hattest sabotiert". **Target-blind Re-Review (Chat c327241a): GO 87 (ehrlich!)** + 3 reale MINOR die der ankerte Review überging: toter Tipp-Button im Finale, Grace 900<1150, ▸ blinkt nicht → gefixt (1ff15bf). **LEKTION: NIE Vor-Score/„bestand-schon" im Gate-Prompt (ankert den Gutachter), Referenzen = SAUBERE GO-Spiele.**
- **DEFEKT-AUDIT 03.07. (Bolle-Feedback → `_dev/handoff/2026-07-03-defekt-audit.md`):** Read-only Workflow über alle 45 Spiele (Haiku, dann Haupt-Claude-Triage). Zwei set-weite Klassen: **(1) Blickrichtung** — 7 Kandidaten (3 sichere Flips jeep-safari/korallen/bagger, 4 zu prüfen: stadt-superheld frontal?, hufeisen Wurfobjekt, spuren+faehrte gewundene Pfade); **(2) Attributions-Umkehr** — Win-Zeile lobt kid() für Spieler-Leistung (13 echt, ~14 weiche MINOR, 10 sauber, lianen=Haiku-Fehlflag). Unabhängig vom huerden-Opus-Gutachter bestätigt. **Bolle-Entscheidung: STRIKT EINZELN — beide Defekte pro Spiel im jeweiligen Gate, kein Sweep.** Katalog = Arbeitsliste. Ab sofort jedes Gate-Prompt zusätzlich: Blickrichtung + Attribution prüfen. Prinzip: Gast wird gelobt („Du hast…"), Kind lädt ein/ist enthüllter Star; `kid()+Spiel-Verb`=Fehler, `kid()` als Reveal-Subjekt=ok.
- **EINZELN-LOOP 03.07. (Bolle: „3 parallel, 3 Tabs, Spiel für Spiel", Opus-4.8-Max-Audit maßgeblich):** Jedes Spiel = voller Schmiede-Pass (Story-Muster v2 + Attributions-Fix NOFOTO-winWho + AC()-Unlock), target-blind Gate. **Durch:** #5 gluehwuermchen-feen **GO 84** (Chat bf9812be, Simon; Politur: Konfetti-am-Reveal, FLASH_MS alters-gestaffelt, nofoto-Beat), #6 loeschen-feuerwehr **GO 84** (Re-Gate 2a3b88bb nach MAJOR-Fix: NOFOTO löste falsche Fährte nicht auf → „der Held warst DU" + „Held"→genusneutraler „Star"). **Gate läuft:** #7 regenbogen-einhorn (912cb9c5), #8 sternbild-weltraum (3c958dd1) — beide Connect-Mechanik, svg fadet → Foto flutet. **Audit-Facing bestätigt False-Positive-lastig (Opus):** nur faehrte-dino echter Kandidat (visuell prüfen), jeep/korallen/bagger/stadt = Spurwechsler ohne Laufrichtung, spuren/hufeisen frontal/CSS. **Chrome-MCP-Send (verfeinert):** `el.editor.commands.insertContent(P)` (tiptap-API geht wieder) synct Reacts State NICHT → Klick auf eine ECHTE TEXTZEILE im Editor + eine echte Taste tippen (`.`/Space) → dann Enter. Klick auf leeren Editor-Raum fokussiert ProseMirror nicht.
