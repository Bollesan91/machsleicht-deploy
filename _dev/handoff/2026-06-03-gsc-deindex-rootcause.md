...
1. **Legacy-Redirect-Strategie fixen (höchster Hebel).** Statt „alles 301 → /kindergeburtstag": **410 Gone** für die wirklich toten Thin-/Lizenz-Permutationen (Lizenzmarken kommen nie zurück → 410 ist das sauberste Signal „endgültig weg, aus dem Index nehmen"). 301 NUR dort behalten, wo ein echtes relevantes Ziel existiert. → Redirect-Regeln in `_redirects` / `netlify.toml` prüfen. **Cave:** vorher Liste der Legacy-URLs + ihrer aktuellen Redirect-Ziele ziehen.
