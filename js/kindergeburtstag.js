...
function Connector() {
  return (
    <div style={{ textAlign: "center", padding: "0 0 4px" }}>
      ...
      <p style={{ margin: "4px 0", fontWeight: 600, color: "var(--a)", fontSize: 12 }}>Die Einladung teast die Schatzsuche an</p>
      <div style={{ width: 1, height: 20, background: "var(--l)", margin: "0 auto" }} />
    </div>
  );
}
...
function App() {
  ...
  // === ACTIONS ===
  function reset() { setView("config"); setMottoId(null); setQuietMode(false); setOwned({}); setShoppingMode("standard"); setLocOverride(null); setEmergencyMode(false); }
  function startPlan() { if (mottoId) { setView("peak"); window.scrollTo(0, 0); setTimeout(() => { setView("plan"); window.scrollTo(0, 0); }, 2800); } }
      ...
      <section style={{ position: "relative", overflow: "hidden", padding: "48px 24px 32px", background: "linear-gradient(165deg, #fef8f0 0%, #fdfcf9 40%, #f0f4e8 100%)" }}>
        <div style={{ maxWidth: 660, margin: "0 auto", textAlign: "center", position: "relative", zIndex: 1 }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8, justifyContent: "center", animation: "fadeSlideUp 0.5s 0.3s ease-out both" }}>
            {[["⏱️", "Zeitplan"], ["🎯", "Altersgerechte Spiele"], ["🛒", "Einkaufsliste"], ["🧮", "Kosten pro Kind"]].map(([ico, label]) => (
