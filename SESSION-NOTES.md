...
- OFFEN Welle A: drehleiter, notruf, stadt, wildnis. Welle B: huerden, lianen. + ChatGPT-Zweitwinkel je Spiel/Motto. **ALLE 6 Stufe-3 reveal-last-SELBSTVERIFIZIERT (10.07.): safe** — gedeckelte sharpen (opacity≤.3–.4, blur≥11–14) + `if(magicPhase)return`-guard + winSeq ohne sharpen(1) + catchMagic versteckt Reveal-Element. Nur noch Independent-Review + Copy/Robustheit-MINORs offen (kein reveal-last-Risiko mehr).
...
**🚫 SEND-BLOCKER autonom (10.07. ~00:20, hart diagnostiziert):** claude.ai-Reviews können NICHT autonom gesendet werden, solange Chrome im Hintergrund ist. `document.visibilityState:"hidden"` → synthetische Keystrokes erreichen die ProseMirror NICHT → kein Send. Beide Auswege brauchen Bolles Präsenz: (a) Chrome-Fenster in den OS-Vordergrund holen; (b) computer-use `open_application` braucht `request_access`-Approval-Dialog. **→ Review-Pipeline blockiert bis Bolle Chrome nach vorn holt.** WENN CHROME VORN: ich batche ALLE offenen (drehleiter/notruf/stadt/wildnis/huerden/lianen + strahl-re-confirm + ChatGPT-Zweitwinkel). Solange: Nicht-Send-Arbeit (Stufe-3 reveal-last-Selbstverifikation der gebauten Spiele; Independent-Review bleibt Pflicht vor „gated"/Deploy).
