...
**Roll-out-Stand:** **12 Spiele interaktiv, 4 volle Mottos** = meerjungfrau, feen, einhorn + **weltraum** (funk `ebe7d9b`, rakete `1fa59f0`, sternbild `7adf0bf` — je Flit+Caught per Screenshot verifiziert, caught opacity:1 gesetzt [laterne-Lektion], Reveal-last: rakete-Bullauge + sternbild/cphoto Früh-Flood entfernt). **NÄCHSTE:** detektiv (akte alt-magic, wimmel alt-magic, fingerabdruck kein-magic) + weltraum Einzel-Doppelcheck-Reviews. Screenshot-Freeze: flit setzen + `getAnimations().forEach(a=>{a.currentTime=250;a.pause()})`; Server python http.server 8766 (Root), `--photo`→`/_dev/prototypes/birthday-photo.jpg`. **Container-Muster:** Simon/kein-Stage-Container → magicFly in `#console`/`.space` (position:relative); Simon-Playtest-Artefakt: forciertes win() + pending nextRound überschreibt msg (im echten Spiel egal). **Git-Auth-Panne:** falls `git push` nach Passwort fragt → `git -c credential.helper='!gh auth git-credential' push origin draft`.
