...
- Welle A: **strahl NO-GO 57 → sharpen-MAJOR gefixt `732523f`** (fire() rief sharpen(1) am Sieg-Treffer, Funktion war ungedeckelt; jetzt gekappt; RE-CONFIRM offen) · hufeisen **84 GO** (Reveal-last 100) · **drehleiter 93 GO** („Gate wasserdicht auf allen Pfaden", kein MAJOR) · **notruf 89 GO** („Gate wasserdicht, nichts blockt GO", kein MAJOR).
- OFFEN Welle A: stadt, wildnis (Prompts bereit). Welle B: huerden, lianen. + ChatGPT-Zweitwinkel je Spiel/Motto. **ALLE 6 Stufe-3 reveal-last-SELBSTVERIFIZIERT (10.07.): safe** — gedeckelte sharpen (opacity≤.3–.4, blur≥11–14) + `if(magicPhase)return`-guard + winSeq ohne sharpen(1) + catchMagic versteckt Reveal-Element. Independent bestätigt für drehleiter+notruf (Selbst-Verify war akkurat).
...
**🚫 SEND-BLOCKER autonom (10.07., verfeinert): das Chrome-Fenster muss im OS-Vordergrund BLEIBEN (nicht nur kurz antippen).** Der Send braucht EINEN echten, vertrauenswürdigen Keystroke — der landet nur bei `visibilityState:"visible"`. **Bewährtes Rezept (verifiziert an drehleiter+notruf):** execCommand-insert Prompt → in Composer-Textzeile klicken → **`computer type " "`** (EIN echtes Leerzeichen; synct Reacts State → Send-Button „Nachricht senden" wird enabled) → **JS `[...buttons].find(b=>/senden/i.test(b.ariaLabel)&&!b.disabled).click()`** (umgeht Enter-Fokus-Flakiness). BESTÄTIGT WIRKUNGSLOS: synthetische `InputEvent`/`KeyboardEvent`/`beforeinput` (nicht isTrusted → Button erscheint nie); tiptap `editor.commands.insertContent(' ')` (füllt Doc, triggert Reacts onChange NICHT). Guthaben-Banner + Modell (Opus 4.8 Max) vorher checken. **drehleiter+notruf gingen im kurzen Vordergrund-Fenster nach Bolles „cursor"-Nudge raus; danach Fenster wieder hinten → stadt/wildnis/huerden/lianen/strahl warten, bis das Fenster vorne BLEIBT.** Lesen der Verdikte (navigate+screenshot) geht auch im Hintergrund.
